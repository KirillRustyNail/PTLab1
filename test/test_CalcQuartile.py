...
class TestCalcRating:
    ...
    def test_calc_last_quartile(self, input_data: tuple[DataType,
                                                    float, StudentIn]) -> None:
        calc_rating = CalcQuartile(input_data[0])
        assert input_data[0] == calc_rating.rating

    def test_calc_quartile(self, input_data: tuple[DataType, float, StudentIn]) -> None:

        rating = CalcQuartile(input_data[0]).calc_last_quartile()
        assert pytest.approx(rating,
                             abs=0.001) == input_data[1]

    def test_shows_calc_quartile(self, input_data: tuple[DataType,
                                                        float, StudentIn]) -> None:

        rating_scores: StudentIn = CalcQuartile(input_data[0]).\
